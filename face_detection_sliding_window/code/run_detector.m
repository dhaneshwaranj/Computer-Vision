...
%     fprintf('Detecting faces in %s\n', test_scenes(i).name)
    ...
    [orig_row, orig_col] = size(img);
    for scale=1:-0.2:0.2
        image = imresize(img, scale, 'bilinear');
        hog_features = vl_hog(image, cellsize);
        [row, col] = size(image);
        if (row<template) || (col<template)
            break;
        end
        for r=1:cellsize:row-template
            y = ceil(r/cellsize);
            for c=1:cellsize:col-template
               x = ceil(c/cellsize);
               test_features = hog_features(y:y+window-1,x:x+window-1,:);
               test_features = reshape(test_features, [1, D]);
               regress = test_features*w + b;
               if regress > threshold
                   xmin = round((c/col)*orig_col);
                   xmax = round(((c+template-1)/col)*orig_col);
                   ymin = round((r/row)*orig_row);
                   ymax = round(((r+template-1)/row)*orig_row);
                   cur_bboxes = [cur_bboxes; [xmin, ymin, xmax, ymax]];
                   cur_confidences = [cur_confidences; regress];
                   cur_image_ids(size(cur_image_ids,1)+1,1) = {test_scenes(i).name};
               end
           end
        end
    end
