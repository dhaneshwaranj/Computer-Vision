...
for i=1:num_images
    ...
    hog_features = reshape(vl_hog(img, cellsize), [1, D]);
    features_pos(i,:) = hog_features;
    img1 = flip(img, 2);
    hog_features = reshape(vl_hog(img1, cellsize), [1, D]);
    features_pos(i+num_images,:) = hog_features;
%     for d=3
%         im = imrotate(img,d,'crop');
%         hog_features = reshape(vl_hog(im, cellsize), [1, D]);
%         features_pos(i+(1+(d/3))*num_images,:) = hog_features;
%         im = imrotate(img,-d,'crop');
%         hog_features = reshape(vl_hog(im, cellsize), [1, D]);
%         features_pos(i+(2+(d/3))*num_images,:) = hog_features;
%         im = imrotate(img1,d,'crop');
%         hog_features = reshape(vl_hog(im, cellsize), [1, D]);
%         features_pos(i+(3+(d/3))*num_images,:) = hog_features;
%         im = imrotate(img1,-d,'crop');
%         hog_features = reshape(vl_hog(im, cellsize), [1, D]);
%         features_pos(i+(4+(d/3))*num_images,:) = hog_features;
%     end
end
