...
for i=1:size(image_paths,1)
    ...
    img1 = imresize(img, 0.75, 'bilinear');
    img2 = imresize(img, 0.5, 'bilinear');
    img3 = imresize(img, 0.25, 'bilinear');

    [locations, SIFT_features] = vl_dsift(img,'Step', step);
    features = [features,  SIFT_features];
    [locations, SIFT_features] = vl_dsift(img1,'Step', step);
    features = [features,  SIFT_features];
    [locations, SIFT_features] = vl_dsift(img2,'Step', step);
    features = [features,  SIFT_features];
    [locations, SIFT_features] = vl_dsift(img3,'Step', step);
    features = [features,  SIFT_features];
end
...
[centers, assignments] = vl_kmeans(single(features), vocab_size);
vocab = centers';
